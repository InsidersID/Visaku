    ...
    class CameraView: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate, AVCapturePhotoCaptureDelegate {
        ...
        private func configureFaceDetectionRequest() {
            faceDetectionRequest = VNDetectFaceLandmarksRequest { [weak self] request, error in
                ...
                guard let results = request.results as? [VNFaceObservation], let face = results.first else {
                    DispatchQueue.main.async {
                        self.cameraState.isHeadCentered = false
                    }
                    print("No face detected.")
                    return
                }
                self.analyzeHeadPosition(face: face)
            }
        }
        
        private func analyzeHeadPosition(face: VNFaceObservation) {
            ...
            if abs(yaw) < 0.2 {
                DispatchQueue.main.async {
                    self.cameraState.isHeadCentered = true
                }
                print("Head is centered")
            } else {
                DispatchQueue.main.async {
                    self.cameraState.isHeadCentered = false
                }
                print("Head is not centered")
            }
        }
        
        override func viewDidLoad() {
            super.viewDidLoad()
            configureFaceDetectionRequest()
            checkPermission()
        ...
        func monitorHeadCentered() {
            guard let camera = videoDevice else { return }
            
            DispatchQueue.main.async {
                camera.addObserver(self, forKeyPath: "yaw", options: [.new, .old], context: nil)
                camera.addObserver(self, forKeyPath: "pitch", options: [.new, .old], context: nil)
            }
        }
        ...
        func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
            ...
            if cameraState.isFocused && cameraState.shouldCaptureImage && cameraState.isHeadCentered && cameraState.offset > -0.25 {
    ...
    //                onDismiss()
        ...
        deinit {
            ...
            videoDevice?.removeObserver(self, forKeyPath: "yaw")
            videoDevice?.removeObserver(self, forKeyPath: "pitch")
        }
    }
